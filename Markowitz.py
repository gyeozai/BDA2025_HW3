...
class RiskParityPortfolio:
    ...
    def calculate_weights(self):
        ...
        """
        for date in self.portfolio_weights.index:
            loc_current_date_in_df = df.index.get_loc(date)

            if loc_current_date_in_df < self.lookback + 1:
                continue

            start_date = df.index[loc_current_date_in_df - self.lookback]
            end_date = df.index[loc_current_date_in_df - 1]
            window_returns = df_returns.loc[start_date:end_date][assets]
            ...
            # 2. Complex version (can handle invalid cases)
            # weights = pd.Series(float('nan'), index=assets)

            # valid_vol_mask = vol.notna()            
            # if valid_vol_mask.any():
            #     vol_valid = vol[valid_vol_mask]
            #     assets_valid = vol_valid.index

            #     is_zero_vol_valid = (vol_valid == 0)
            #     num_zero_vol_valid = is_zero_vol_valid.sum()

            #     if num_zero_vol_valid == len(vol_valid): 
            #         # Case 1: All assets with valid vol have zero volatility
            #         if len(vol_valid) > 0 :
            #             weights[assets_valid] = 1.0 / len(vol_valid)
            #     elif num_zero_vol_valid > 0: 
            #         # Case 2: Some assets (valid vol) have zero volatility, others positive
            #         zero_vol_assets_in_valid_subset = assets_valid[is_zero_vol_valid]
            #         weights[zero_vol_assets_in_valid_subset] = 1.0 / num_zero_vol_valid
            #         positive_vol_assets_in_valid_subset = assets_valid[~is_zero_vol_valid]
            #         weights[positive_vol_assets_in_valid_subset] = 0.0
            #     else: 
            #         # Case 3: All assets with valid vol have positive (non-zero, non-NaN) volatility
            #         inv_vol_valid = 1.0 / vol_valid
            #         sum_inv_vol_valid = inv_vol_valid.sum()
            #         if sum_inv_vol_valid != 0 and pd.notna(sum_inv_vol_valid):
            #             weights[assets_valid] = inv_vol_valid / sum_inv_vol_valid
            
            # 3. Fill the weights for the current date
...
class Helper:
    ...
    def plot_mean_variance_allocation(self):
        ...
        self.plot_allocation(self.mv_list[2][0])
        self.plot_allocation(self.mv_list[3][0])
